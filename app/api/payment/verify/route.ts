...
export async function GET(req: Request) {
  ...
  // Dynamic Base URL for Vercel redirects
...
async function handleRegistrationPayment(data: any) {
  ...
  await prisma.payment.create({
    data: {
      ...
      channel: data.channel,
      status: data.status,
      // Fixed: type check for adminNote
      adminNote:
        data.metadata.type === "REGISTRATION"
          ? "Registration Fee"
          : "General Payment",
    },
  });
...
async function handleDashboardPayment(data: any) {
  ...
  // CBT LOGIC REMOVED/COMMENTED OUT TO PREVENT BUILD ERRORS
  /* if (type === "CBT") {
     // ... CBT code
  }
  */
  ...
  const receiptData = {
    ...
    amount: data.amount / 100,
    amountInWords: convertToWords(data.amount / 100),
    description: type === "TUITION" ? "Monthly Tuition Fee" : "School Fee",
    dueDate:
      type === "TUITION"
        ? new Date(
            new Date().setMonth(new Date().getMonth() + 1),
